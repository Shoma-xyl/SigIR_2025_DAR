...
class CTRTrainer(object):
    ...
    def train_one_epoch(self, data_loader, log_interval=10):
        ...
        for i, (x_dict, y) in enumerate(tk0):
            ...
            if self.use_ot or self.use_independence_loss:
                y_pred, loss_dict = self.model(x_dict)
            else:
                y_pred = self.model(x_dict)
            loss = self.criterion(y_pred, y.float())
            self.model.zero_grad()
            if self.use_ot:
                loss = loss + loss_dict['loss_c'] + loss_dict['loss_s']
            if self.use_independence_loss:
                loss = loss + loss_dict['loss_independence']
            loss.backward()
            self.optimizer.step()
    ...
    def fit(self, train_dataloader, val_dataloader=None):
        ...
        name = self.model.__class__.__name__ + "__" + "ptr" + "_" +self.data_set_type+"_"+ time_now + ".pth"
    ...
    def evaluate_multi_domain_loss(self, model, data_loader,domain_num):
        ...
        for d in range(domain_num):
            # print("it is ok")
            domain_d_logloss_val = log_loss(targets_domain_specific_list[d], predicts_domain_specific_list[d],labels=[0, 1]) if targets_domain_specific_list[d] else None#做purchase时很容易所有y都是0故添加labels=[0, 1]
